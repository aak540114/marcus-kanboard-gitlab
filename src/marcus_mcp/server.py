        ...
        async def gate_setting_api(request: Request) -> JSONResponse:
            ...
            if request.method == "PUT":
                ...
                if path.endswith("/project"):
                    ...
                    verify_count = body.get("verify_count")  # may be int or None to reset; absent = don't touch
            ...
            project_verify_count = gate_mgr.get_project_verify_count(pid) if pid is not None else None
            ticket_verify_count = gate_mgr.get_ticket_verify_count(tid_str) if tid_str else None
            effective_verify_count = (
                gate_mgr.get_effective_verify_count(tid_str, pid)
                if pid is not None and tid_str
                else (
                    ticket_verify_count
                    if ticket_verify_count is not None
                    else (project_verify_count or 0)
                )
            )
            ...
            r = JSONResponse({
                ...
                "effective": effective,
                "project_verify_count": project_verify_count,
                "ticket_verify_count": ticket_verify_count,
                "effective_verify_count": effective_verify_count,
            })
            r.headers["Access-Control-Allow-Origin"] = "*"
            return r
